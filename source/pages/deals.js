...
const Deals = () => {
  ...
  const [queryParam, setQueryParam] = useState(router);
  ...
  const [facetObj, setFacetObj] = useState(false);
  // adding checked key to all facet Objects
  ...
  const getSearchResult = async (searchKey, fromFacetBadge, fromMenu) => {
    ...
    if (searchKey) {
      searchResult = await API.getLogSearchService(
        formatMessage(
          config.apiUrlData.externalData[API.getSysEnv()].part_url,
          {
            collectionId: solrCollection,
            dealParam: router.query.dealParam,
            searchString: "*:*",
            facet:
              facetJSON === []
        ...
        "promotions", // filteredSearchTerm, //searchTerm for signal
      ...
      //         dealParam: fragment,
      //         searchString: '*:*',
      //         facet: JSON.stringify(newFacetArray),
      //         sortedKey: sortedKey,
      //       },
      //     ),
      //     null, //productid for signal
      //     'promotions', //searchTerm.toLowerCase() for signal
      //     facetTerm, //.length > 0 ? facetTerm.length[facetTerm.length-1]:"", //facetTerm for signal
      //     signalType,
      //     null,
      //     null, // categoryId for signal
    }
    ...
    //       facet: JSON.stringify(facetJSON),
    //       sortedKey: sortKey,
    //     })
    //   );
    // }
    ...
    if (searchResult?.response?.data?.response?.docs.length > 0) {
      ...
      if (fromFacetBadge === false) {
        ...
        for (let i = 0; i < facetsArr.length; i++) {
          ...
          newFacetObj[facetField].forEach((foo) => (foo.checked = false));
        }
      ...
      // setFacetFilterData(null);
    }
  };

  const getQueryCheckedValues = (data, query, title) => {
    // console.log("getQueryCheckedValues", { data, query, title });
    const queryArr = getParams(router.query, query);
    const modData = {
      [title]: data.map((item) => {
        if (title === item.facetName) {
          item.checked = true;
        } else {
          item.checked = false;
        }
  ...
  const onFacetChangeHandler = (facetName, title) => {
        ...
        if (item.facetName === facetName) {
          ...
          if (item.checked === true) {
            ...
            let index = selectedFacets[title].findIndex(
              (foo) => foo.field_value === item.facetName
            );
            if (index > -1) {
              // facet not found
    ...
    getSearchResult(router.query.dealParam, true);
  ...
  const badgeClickHandler = () => {
    ...
    // remove badge from selectedFacets
  };

  const sendSearchSignal = () => {};
  // set or get query params
  useEffect(() => {
    // setQueryParam(new URLSearchParams(router.query).get("search"));
    if (router) {
      if (router.query.dealParam) {
        // setSearchTerm(location.state.searchTerm);
        setSearchTerm(router.query.dealParam);
      }
      // if (queryParam) {
      //   setSearchTerm(queryParam);
      // }
      // getSearchResult(router.query.dealParam, false);
    }
    getSearchResult(router.query.dealParam, false);
  }, [searchTerm, queryParam, router]);
  ...
  useEffect(() => {
    setSearchTerm(router.query.dealParam);
    // setSelectedFacets(router.query.facetField);
    // if (router.query.dealParam) {
    //   setSearchTerm(router.query.dealParam);
    // } else {
    //   setSelectedFacets({ ...router.query.facetField });
  ...
  useEffect(() => {
    getSearchResult(router.query.dealParam, true, false);
  }, [sortKey]);

  // useEffect(() => {
  //   if (setFacetObj === null) {
  //     if (router.query.facetObj) {
  //       setFacetObj(router.query.facetObj);
  //       getSearchResult(
  //         router.query.dealParam,
  //         false, // from facet
  //         true // from menu
  //       );
  //     }
  ...
  //     router.asPath.includes("L1Category")
  //   ) {
  //     const modifiedData0 = onFacetChangeHandler(
  //       facetFilterData[Object.keys(facetFilterData)[0]],
  //       "Brand",
  //       new URLSearchParams(router.asPath).get("Brand")
  //       // Object.keys(facetFilterData)[0]
  //     );
  //     const modifiedData1 = onFacetChangeHandler(
  //       facetFilterData[Object.keys(facetFilterData)[1]],
  //       "L1Category",
  //       new URLSearchParams(router.asPath).get("L1Category")
  //       // Object.keys(facetFilterData)[1]
  //     );
  //     //   modifiedData0,
  //     //   modifiedData1,
  //     // ]);
  //     // setFacetFilterData([modifiedData0, modifiedData1]);
  //   }
  // }, []);
            ...
            <div className="fixed inset-0 flex bg-black bg-opacity-25">
              <div className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-12 shadow-xl">
                ...
                <form className="mt-4 border-t border-gray-200">
                  ...
                  {!isNullOrUndefined(facetFilterData) &&
                    Object.keys(facetFilterData).map((foo) => {
                      return (
                        <FacetFilter
                          key={foo}
                          type={"checkbox"}
                          title={foo}
                          data={facetFilterData[foo]}
                          maxShow={5}
                          preset={true}
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-4">
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            {!isNullOrUndefined(selectedFacets) &&
              Object.keys(selectedFacets).map((foo) => {
                if (foo.length > 0) {
                  return selectedFacets[foo].map((buzz) => {
                    return (
                      <button
                        key={buzz}
                        type="button"
                            ...
                            {noDataBoo?.noDataObj?.suggestions[1]?.suggestion.map(
                              (foo) => (
                                <li
                                  ...
                                  onClick={() =>
                                    router.push(`/search/?search=${foo.word}`)
                                  }
                                >
                                  {foo.word}
                                </li>
